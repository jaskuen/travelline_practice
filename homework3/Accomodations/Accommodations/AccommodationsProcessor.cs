    ...
    private static void ProcessCommand(string input)
    {
        ...
        switch (commandName)
        {
            case "book":
                ...
                if (!Enum.TryParse(parts[5], true, out CurrencyDto currency)) // парсим валюту
                {
                    throw new ArgumentException("Wrong currency was entered");
                }
                if (!DateTime.TryParse(parts[3], out DateTime startDate)) // парсим начальную дату
                {
                    throw new ArgumentException("Date of the book beginning is incorrect");
                }
                if (!DateTime.TryParse(parts[4], out DateTime endDate)) // парсим конечную дату
                {
